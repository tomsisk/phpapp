...
Control.ColorPicker.prototype = {
	initialize: function (element, options) {
		...
		//if (!this.element.value) this.element.value = '#FFFFFF';

		var cont = new Element('div', {'style': 'position: relative;'});
		this.element.parentNode.insertBefore(cont, this.element);
		cont.appendChild(this.element);

		this.swatch = document.createElement('div');
		var top = '3px';
		var size = (this.element.offsetHeight - 8);
		var left = (this.element.offsetWidth - (size + 5)) + 'px';
		...
		Event.observe(element, 'change', this.textChanged.bindAsEventListener(this));
		Event.observe(element, 'blur', this.hidePicker.bindAsEventListener(this));
	...
	colorSelected: function(color) {
		...
		if (this.element.onchange)
			setTimeout(this.element.onchange.bind(this.element));
	},
